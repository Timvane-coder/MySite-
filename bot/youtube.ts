...
// Helper function to try multiple lyrics APIs
const tryMultipleLyricsAPIs = async (artist, song, originalQuery) => {
    const apis = [
        () => tryLyricsOvhAPI(artist, song),
        () => tryLyristAPI(artist, song),
        () => tryYouTubeMusicSearch(originalQuery)
    ];
    ...
    for (const apiCall of apis) {
        try {
            ...
            console.log(`Lyrics API failed: ${error.message}`);
            continue;
        }
    }
...
// Lyrics.ovh API
const tryLyricsOvhAPI = async (artist, song) => {
...
// Lyrist API (alternative)
const tryLyristAPI = async (artist, song) => {
    if (!artist || !song) return null;
    
    try {
        const response = await axios.get(`https://lyrist.vercel.app/api/${encodeURIComponent(song)}/${encodeURIComponent(artist)}`, {
            timeout: 5000
        });
        
        if (response.data && response.data.lyrics) {
            return {
                title: response.data.title || song,
                artist: response.data.artist || artist,
                lyrics: response.data.lyrics,
                thumbnail: response.data.image || null,
                source: 'lyrist'
            };
        }
    } catch (error) {
        throw new Error('Lyrist API failed');
    }
