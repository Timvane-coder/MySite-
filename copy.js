...
//OWNER MENU
case 'shop':
  var shopData = JSON.parse(fs.readFileSync('./media/game/shop.json'));

  for (let item of shopData) {
    let shopText = `🛍️ *${item.name}*\n`;
    shopText += `💰 *Price:* ${item.price.toLocaleString()} coins\n`;
    shopText += `📜 ${item.description}\n\n`;
    shopText += `🔹 Use *buy ${item.index}* to purchase.`;

    let buttons = [{
      buttonId: `buy ${item.index}`,
      buttonText: { displayText: `Buy ${item.name}` },
      type: 1
    }];
...
case 'buy':
  if (!args[0] || isNaN(args[0])) return m.reply('🔹 Use *buy <ID>* to purchase an item.');

  let shopItems = JSON.parse(fs.readFileSync('./media/game/shop.json'));
  //let users = global.db.data.users[m.sender];  // Access user data from global database

  if (!users) return m.reply('❌ User data not found.');
  if (!users.inventory) users.inventory = {}; // Ensure inventory exists

  let itemId = parseInt(args[0]);
  let item = shopItems.find(i => i.index === itemId);

  if (!item) return m.reply(`⚠️ Invalid item ID. Use *shop* to see available items.`);
  if (users.balance < item.price) return m.reply(`❌ Insufficient coins! You need 💰 ${item.price - users.balance} more.`);
  ...
  // No need to manually save global.db; it should automatically update
  m.reply(`✅ You purchased *${item.name}* for 💰 ${item.price} coins.\n🎒 Check *inventory* to view your items.`);
  break;

case 'inventory':
  //let users = global.db.data.users[m.sender];  // Access user data
  let shopItem = JSON.parse(fs.readFileSync('./media/game/shop.json'));

  if (!users || !users.inventory || Object.keys(users.inventory).length === 0) {
    return m.reply("❌ Your inventory is empty.\n🔹 Use *shop* to view available items and *buy <ID>* to purchase one.");
  }
  ...
  for (let item in users.inventory) {
    let itemInfo = shopItem.find(i => i.name.toLowerCase().replace(/\s+/g, "_") === item);
    ...
    if (itemInfo) {
      ...
      invText += `🔹 *Unknown Item* - ${users.inventory[item]}x (Item not found in shop.json)\n\n`;
    }
  }

  // Append a hint on how to use items
  invText += `🔹 To use an item, type *use <item>*.\nFor example: *use extra time*.\n`;
  invText += `🔹 To purchase new items, type *shop*.\n`;
...
case 'use':
  ...
  // Combine the arguments to form the item name and convert to our key format
  let itemName = args.join("_").toLowerCase();
  // Access the user's inventory from the global database
  ...
  // Make sure an ongoing game exists for this chat
  if (!(from in tebakgambar2)) {
    return m.reply('❌ You can only use items during an active Tebak Gambar game.');
  }
  
  // Now apply the item effect
  switch (itemName) {
    case "extra_time":
      // Add extra time: clear the current timeout and extend the time by 10 seconds
    ...
    case "skip_question":
      // Skip the current question and clear the session
      let skippedAnswer = tebakgambar2[from].jawaban;
      delete tebakgambar2[from];
      m.reply(`⏭️ *Question Skipped!* The correct answer was: *${skippedAnswer}*`);
      break;
      
    case "auto_guess":
      // Automatically show the answer and end the game
      m.reply(`🤖 *Auto Guess Activated!* The correct answer is: *${tebakgambar2[from].jawaban}*`);
      delete tebakgambar2[from];
      break;
      
    case "double_reward":
      // Double the reward for this round (prevent stacking by checking a flag)
      if (tebakgambar2[from].doubleUsed) {
        return m.reply('⚠️ *Double Reward* has already been used for this round.');
      }
      tebakgambar2[from].hadiah *= 2;
      tebakgambar2[from].doubleUsed = true;
      m.reply('💰 *Double Reward Activated!* Your reward for this round is doubled.');
      break;
      
    case "auto_reveal_hint":
      // Reveal part of the answer as a hint (prevent multiple uses)
      if (tebakgambar2[from].hintUsed) {
        return m.reply('⚠️ *Auto Reveal Hint* has already been used for this round.');
      }
  ...
  // Reduce the quantity of the used item in the user's inventory
...
case 'tebak':
  ...
  let gameData = JSON.parse(fs.readFileSync('./media/game/tebakgambar.json'));
  if (!gameData || gameData.length === 0) return m.reply('⚠️ No game data available.');

  // Pick a random game question
  var { img, jawaban, deskripsi } = pickRandom(gameData);
  console.log('Jawaban : ' + jawaban);

  // Access user data from the global database
  // let users = global.db.data.users[m.sender];
  // Load shop items to cross-reference inventory item names
  let shop = JSON.parse(fs.readFileSync('./media/game/shop.json'));
  let userItem = users.inventory || {};

  // Build a list of available items (formatted using shop data)
  let availableItem = Object.keys(userItem)
    .filter(key => userItem[key] > 0)
    .map(key => {
      let itemInfomation = shop.find(i => i.name.toLowerCase().replace(/\s+/g, "_") === key);
      return itemInfomation ? `🔹 ${itemInfomation.name} (${userItem[key]}x)` : null;
    }).filter(Boolean);

  let itemText = availableItem.length > 0 
    ? `🎒 *Your Items Available to Use*\n${availableItem.join("\n")}\n\nUse: *use <item>*\nExample: *use extra time*`
    : `❌ No items available.\n🔹 Use *shop* to view and *buy* items.`;

  // Build the game message with the image, clue, description, and item hints
  ...
  // Start the game session by saving game details
  tebakgambar2[from] = {
    soal: img,
...
case "electronic": {
const url1 = `https://jet-ocelot-7393.twil.io/assets/iphone.jpeg`;
const url2 = `https://jet-ocelot-7393.twil.io/assets/tv.jpeg`;
const url3 = `https://jet-ocelot-7393.twil.io/assets/samsung.jpeg`;
const url4 = `https://jet-ocelot-7393.twil.io/assets/fridge.jpeg`;
const url5 = `https://jet-ocelot-7393.twil.io/assets/genesis.jpg`;
const url6 = `https://jet-ocelot-7393.twil.io/assets/samsung.jpeg`;
...
let msg = generateWAMessageFromContent(
 ...
 message: {
 interactiveMessage: {
 body: { text: `Hello customer ${pushname}, welcome to electronic shop items available :5` },
 carouselMessage: {
 cards: [
 {
 header: {
 ...
 body: { text: "*- 1 Iphone 16 Pro -*\n\n• Price: 175k" },
 nativeFlowMessage: {
 buttons: [
 {
 name: "cta_url",
 buttonParamsJson: '{"display_text":"Buy Now","url":"https://wa.me/265986919367?text=Hello+Iam+Interested+to+buy+iphone","webview_presentation":null}',
 },
 ],
 },
 },
 ...
 body: { text: "*- 2 LG Smart Tv -*\n\n• Price: 10k" },
 nativeFlowMessage: {
 buttons: [
 {
 name: "cta_url",
 buttonParamsJson: '{"display_text":"Buy Now","url":"https://wa.me/265986919367?text=Hello+Iam+Interested+to+buy+Tv","webview_presentation":null}',
 },
 ],
 },
 },
 ...
 body: { text: "*- 3 Samsung S24 -*\n\n• Price: 10k" },
 nativeFlowMessage: {
 buttons: [
 {
 name: "cta_url",
 buttonParamsJson: '{"display_text":"Buy Now","url":"https://wa.me/265986919367?text=Hello+Iam+Interested+to+buy+Samsung","webview_presentation":null}',
 },
 ],
 },
 },
 ...
 body: { text: "*- 4 Upright Fridge -*\n\n• Price: 110k" },
 nativeFlowMessage: {
 buttons: [
 {
 name: "cta_url",
 buttonParamsJson: '{"display_text":"Buy Now","url":"https://wa.me/265986919367?text=Hello+Iam+Interested+to+buy+fridge","webview_presentation":null}',
 },
 ],
 },
 },
 ...
 {
 name: "cta_url",
 buttonParamsJson: '{"display_text":"Buy Now","url":"https://wa.me/265986919367?text=Hello+Iam+Interested+to+buy+Genesis","webview_presentation":null}',
 },
 ],
 },
 },
 {
 header: {
 imageMessage: await image(url6),
 hasMediaAttachment: true,
 },
 body: { text: "*- 6 Samsung ultra -*\n\n• Price: 200k" },
 nativeFlowMessage: {
 buttons: [
 {
 name: "cta_url",
...
case "market": {
const url1 = `https://jet-ocelot-7393.twil.io/assets/f.jpg`;
const url2 = `https://jet-ocelot-7393.twil.io/assets/e.jpg`;
const url3 = `https://jet-ocelot-7393.twil.io/assets/c.jpg`;
const url4 = `https://jet-ocelot-7393.twil.io/assets/b.jpg`;
const url5 = `https://jet-ocelot-7393.twil.io/assets/a.jpg`;
const url6 = `https://jet-ocelot-7393.twil.io/assets/d.jpg`;
const url7 = `https://jet-ocelot-7393.twil.io/assets/g.jpg`;
...
let msg = generateWAMessageFromContent(
 ...
 message: {
 interactiveMessage: {
 body: { text: `Hello  customer ${pushname}, welcome to the marketplace available items :7` },
 carouselMessage: {
 cards: [
 {
 header: {
 ...
 {
 name: "cta_url",
 buttonParamsJson: '{"display_text":"Buy Now","url":"https://wa.me/265986919367?text=Hello+Iam+Interested+to+buy+Dior","webview_presentation":null}',
 },
 ],
 },
 },
 ...
 body: { text: "*- 2 Loboutin -*\n\n• Price: 10k" },
 nativeFlowMessage: {
 buttons: [
 {
 name: "cta_url",
 buttonParamsJson: '{"display_text":"Buy Now","url":"https://wa.me/265986919367?text=Hello+Iam+Interested+to+buy+loboutin","webview_presentation":null}',
 },
 ],
 },
 },
 ...
 body: { text: "*- 3 Givenchy -*\n\n• Price: 10k" },
 nativeFlowMessage: {
 buttons: [
 {
 name: "cta_url",
 buttonParamsJson: '{"display_text":"Buy Now","url":"https://wa.me/265986919367?text=Hello+Iam+Interested+to+buy+givenchy","webview_presentation":null}',
 },
 ],
 },
 },
 ...
 {
 name: "cta_url",
 buttonParamsJson: '{"display_text":"Buy Now","url":"https://wa.me/265986919367?text=Hello+Iam+Interested+to+buy+celine","webview_presentation":null}',
 },
 ],
 },
 },
 ...
 {
 name: "cta_url",
 buttonParamsJson: '{"display_text":"Buy Now","url":"https://wa.me/265986919367?text=Hello+Iam+Interested+to+buy+channel","webview_presentation":null}',
 },
 ],
 },
 },
 ...
 {
 name: "cta_url",
 buttonParamsJson: '{"display_text":"Buy Now","url":"https://wa.me/265986919367?text=Hello+Iam+Interested+to+buy+prada","webview_presentation":null}',
 },
 ],
 },
 },
 ...
 {
 name: "cta_url",
 buttonParamsJson: '{"display_text":"Buy Now","url":"https://wa.me/265986919367?text=Hello+Iam+Interested+to+buy+LV","webview_presentation":null}',
 },
 ],
 },
 },
