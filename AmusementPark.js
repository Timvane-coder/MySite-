...
const AmusementPark = () => {
  ...
  useEffect(() => {
    ...
    //createTowerCrane(scene, { x: -60, y: 2.5, z: -30 }); // Add tower crane here
    const towerCrane = createEnhancedTowerCrane(scene);
    createTrees(scene); 
    setupCraneControls(scene);
   ...
   //let towerCrane;
    ...
    scene.traverse((object) => {
      ...
      //} else if (object.name === 'towerCrane') {    // Add this check for tower crane
        //towerCrane = object;
  ...
  if (cleanWaterFlow) {
    ...
    cleanWaterFlow.visible = flowCyclePosition < flowActiveTime;
     ...
     //if (towerCrane) {
     //animateTowerCrane(towerCrane, delta);
    
    // Automated crane operation demonstration
     //craneOperationTime += delta;

         // Create a sequence of operations for the crane
    //if (craneOperationState === 'rotate') {
      //if (craneOperationTime > 10) { // After 10 seconds, rotate to 180 degrees
        //rotateCrane(towerCrane, Math.PI); // 180 degrees
        //craneOperationState = 'moveTrolley';
        //craneOperationTime = 0;
      //}
    //} else if (craneOperationState === 'moveTrolley') {
      //if (craneOperationTime > 5) { // After 5 seconds, move trolley to position 40
        //moveTrolley(towerCrane, 40);
        //craneOperationState = 'lowerHook';
        //craneOperationTime = 0;
      //}
    //} else if (craneOperationState === 'lowerHook') {
      //if (craneOperationTime > 5) { // After 5 seconds, lower hook to height 40
        //setHookHeight(towerCrane, 40);
        //craneOperationState = 'wait';
        //craneOperationTime = 0;
      //}
   // } else if (craneOperationState === 'wait') {
     // if (craneOperationTime > 8) { // After 8 seconds, start cycle over
        // Reset crane to starting position
       // rotateCrane(towerCrane, 0);
        //moveTrolley(towerCrane, 20);
        //setHookHeight(towerCrane, 20);
       // craneOperationState = 'rotate';
        //craneOperationTime = 0;
        
        // Add a small impulse to the hook for a realistic swing
        //towerCrane.userData.hookSwingVelocityX = (Math.random() - 0.5) * 0.5;
        //towerCrane.userData.hookSwingVelocityZ = (Math.random() - 0.5) * 0.5;
      //}
    //}
  //}
  ...
  // Set some initial animation for the crane
  if (crane) {
    // Rotate the crane initially to 45 degrees
    rotateCrane(crane, 45);
    
    // Move the trolley to a middle position
    moveTrolley(crane, 30);
    
    // Set an initial hook height
    setHookHeight(crane, 20);
    
    // Example of crane animation sequence
    setTimeout(() => rotateCrane(crane, 120), 5000);
    setTimeout(() => moveTrolley(crane, 45), 8000);
    setTimeout(() => setHookHeight(crane, 40), 12000);
  }
    ...
    // Add a small vertical bounce
      ...
      if (crane) {
      animateEnhancedTowerCrane(crane, delta);
      }
...
function setupCraneControls(scene) {
  const crane = scene.getObjectByName('towerCrane');
  if (!crane) return;
  
  // This could be integrated with GUI controls or keyboard controls
  // Example with keyboard controls:
  document.addEventListener('keydown', (event) => {
    switch (event.key) {
      case 'ArrowLeft':
        // Rotate crane counterclockwise
        crane.userData.targetRotation -= Math.PI / 18; // 10 degrees
        break;
      case 'ArrowRight':
        ...
        break;
      case 'ArrowUp':
        // Move trolley outward
        moveTrolley(crane, crane.userData.trolleyPosition + 3);
        break;
      case 'ArrowDown':
        // Move trolley inward
        moveTrolley(crane, crane.userData.trolleyPosition - 3);
      ...
      case 'PageUp':
        // Raise hook
        setHookHeight(crane, crane.userData.hookHeight - 3);
        break;
      case 'PageDown':
        // Lower hook
        setHookHeight(crane, crane.userData.hookHeight + 3);
...
// Example function to demonstrate crane lifting an object
function demonstrateCraneLift(scene) {
  const crane = scene.getObjectByName('towerCrane');
  if (!crane) return;
  
  // Create an object to lift (example: construction material)
  const liftObjectGeometry = new THREE.BoxGeometry(4, 2, 4);
  const liftObjectMaterial = new THREE.MeshStandardMaterial({ color: 0x999999 });
  const liftObject = new THREE.Mesh(liftObjectGeometry, liftObjectMaterial);
  liftObject.position.set(-55, 1, -35); // Position near the crane
  liftObject.castShadow = true;
  liftObject.name = 'constructionMaterial';
  scene.add(liftObject);
  
  // Create a sequence to demonstrate the crane lifting the object
  const liftSequence = () => {
    // 1. Rotate crane to face the object
    rotateCrane(crane, 210);
    
    setTimeout(() => {
      // 2. Move trolley to be above the object
      moveTrolley(crane, 55);
    }, 3000);
    
    setTimeout(() => {
      // 3. Lower hook to grab the object
      setHookHeight(crane, 45);
    }, 6000);
    
    setTimeout(() => {
      // 4. Attach the object to the crane
      attachObjectToCrane(crane, liftObject);
    }, 9000);
    
    setTimeout(() => {
      // 5. Raise the hook with the object
      setHookHeight(crane, 25);
    }, 12000);
    
    setTimeout(() => {
      // 6. Rotate crane to new position
      rotateCrane(crane, 45);
    }, 15000);
    
    setTimeout(() => {
      // 7. Move trolley to new position
  ...
  // Start the demonstration after a delay
  setTimeout(liftSequence, 5000);
}

// Enhanced Tower Crane implementation for Three.js
// Features:
// - Full 360° rotation
// - Movable hook with physics-based rope swinging
// - Realistic crane components based on the diagram
...
// Animation function for the crane
function animateEnhancedTowerCrane(crane, delta) {
  if (!crane) return;
...
function rotateCrane(crane, angleInDegrees) {
  if (!crane) return;
  // Convert degrees to radians
  const targetRadians = THREE.MathUtils.degToRad(angleInDegrees);
  crane.userData.targetRotation = targetRadians;
}

function moveTrolley(crane, position) {
  if (!crane) return;
  // Clamp position between 5 and the jib length minus 5
  crane.userData.targetTrolleyPosition = THREE.MathUtils.clamp(position, 5, 65);
}

function setHookHeight(crane, height) {
  if (!crane) return;
  // Clamp height between 5 and 60
  crane.userData.targetHookHeight = THREE.MathUtils.clamp(height, 5, 60);
}

function extendCrane(crane, heightAddition) {
  if (!crane) return;
  crane.userData.isExtending = true;
  crane.userData.targetVerticalPosition = heightAddition;
}

function attachObjectToCrane(crane, object) {
  if (!crane) return;
  crane.userData.attachedObject = object;
  // You would typically implement additional logic here to 
  // determine if the object is within reach of the hook
}
...
// Demo sequence for the crane

 // Add this function within the AmusementPark component, alongside other creation functions
function createTowerCrane(scene, position = { x: -60, y: 2.5, z: -30 }) {
  // Create a group for the entire crane
  const crane = new THREE.Group();
  crane.name = 'towerCrane';
  crane.position.set(position.x, position.y, position.z);
  ...
  // Materials
  const craneMaterial = new THREE.MeshStandardMaterial({ 
    color: 0xf5a623, // Orange color like in the image
    metalness: 0.7,
    roughness: 0.3 
  });
  ...
  // Add framework details to the tower
  addFrameworkToTower(base, baseWidth, baseHeight, craneMaterial);
  
  // Create the rotating part (turntable)
  const turntable = new THREE.Group();
  turntable.name = 'craneTurntable';
  turntable.position.y = baseHeight;
  crane.add(turntable);
  
  // Add a visible turntable base
  const turntableGeometry = new THREE.CylinderGeometry(baseWidth, baseWidth, 2, 16);
  const turntableMesh = new THREE.Mesh(turntableGeometry, metalMaterial);
  turntable.add(turntableMesh);
  ...
  const counterweight = createCounterweight(metalMaterial);
  counterweight.position.x = -10;
  counterweight.position.y = 2;
  turntable.add(counterweight);
  ...
  // Create the trolley system that moves along the jib
  const trolley = createTrolley(metalMaterial);
  trolley.position.set(20, 0, 0); // Initial position along the jib
  trolley.name = 'craneTrolley';
  jib.add(trolley);
  
  // Create the hook and rope
  const hookAndRope = createHookAndRope(20, metalMaterial);
  hookAndRope.name = 'craneHook';
  trolley.add(hookAndRope);
  ...
  crane.userData = {
    rotationSpeed: 0,
    targetRotation: Math.PI, // Initial target is 180 degrees (in radians)
    hookHeight: 20,
    targetHookHeight: 20,
    trolleyPosition: 20,
    targetTrolleyPosition: 20,
    hookSwingAngleX: 0,
    hookSwingAngleZ: 0,
    hookSwingVelocityX: 0,
    hookSwingVelocityZ: 0
...
// Function to add framework/lattice details to make the tower look more realistic
function addFrameworkToTower(tower, width, height, material) {
  const barSize = 0.5;
  const numSegments = 10;
  const segmentHeight = height / numSegments;
  
  for (let i = 0; i < numSegments; i++) {
    // Add horizontal frames at each segment
    const hFrame1 = new THREE.Mesh(
      new THREE.BoxGeometry(width, barSize, barSize),
      material
    );
    const hFrame2 = new THREE.Mesh(
      new THREE.BoxGeometry(width, barSize, barSize),
      material
    );
    const hFrame3 = new THREE.Mesh(
      new THREE.BoxGeometry(barSize, barSize, width), 
      material
    );
    const hFrame4 = new THREE.Mesh(
      new THREE.BoxGeometry(barSize, barSize, width),
      material
    );
    
    const yPos = i * segmentHeight - height/2 + segmentHeight/2;
    hFrame1.position.set(0, yPos, width/2);
    hFrame2.position.set(0, yPos, -width/2);
    hFrame3.position.set(width/2, yPos, 0);
    hFrame4.position.set(-width/2, yPos, 0);
    
    tower.add(hFrame1, hFrame2, hFrame3, hFrame4);
    
    // Add diagonal cross braces
    if (i < numSegments - 1) {
      // X-Z plane diagonals (front face)
      const diagGeom1 = new THREE.BoxGeometry(Math.sqrt(2) * width, barSize, barSize);
      diagGeom1.translate(0, 0, 0);
      diagGeom1.rotateY(Math.PI/4);
      const diagonal1 = new THREE.Mesh(diagGeom1, material);
      diagonal1.position.y = yPos + segmentHeight/2;
      
      // X-Z plane diagonals (back face)
      const diagGeom2 = new THREE.BoxGeometry(Math.sqrt(2) * width, barSize, barSize);
      diagGeom2.translate(0, 0, 0);
      diagGeom2.rotateY(-Math.PI/4);
      const diagonal2 = new THREE.Mesh(diagGeom2, material);
      diagonal2.position.y = yPos + segmentHeight/2;
      
      tower.add(diagonal1, diagonal2);
    }
  }
}

// Create the cabin for the crane operator
function createCraneCabin(material) {
  const cabin = new THREE.Group();
  cabin.name = 'craneCabin';
  
  // Main cabin body
  const cabinGeometry = new THREE.BoxGeometry(5, 4, 4);
  const cabinMesh = new THREE.Mesh(cabinGeometry, material);
  cabin.add(cabinMesh);
  ...
  // Front window
  const frontWindowGeom = new THREE.BoxGeometry(0.1, 3, 3.5);
  const frontWindow = new THREE.Mesh(frontWindowGeom, glassMaterial);
  frontWindow.position.set(2.05, 0, 0);
  cabin.add(frontWindow);
  
  // Side windows
  const sideWindowGeom = new THREE.BoxGeometry(4.5, 3, 0.1);
  const leftWindow = new THREE.Mesh(sideWindowGeom, glassMaterial);
  leftWindow.position.set(0, 0, 2.05);
  const rightWindow = new THREE.Mesh(sideWindowGeom, glassMaterial);
  rightWindow.position.set(0, 0, -2.05);
  cabin.add(leftWindow, rightWindow);
...
// Create the counterweight
function createCounterweight(material) {
  const counterweightGroup = new THREE.Group();
  counterweightGroup.name = 'craneCounterweight';
  
  // Multiple stacked blocks for the counterweight
  for (let i = 0; i < 5; i++) {
    const blockGeometry = new THREE.BoxGeometry(2, 2, 6);
    const block = new THREE.Mesh(blockGeometry, material);
    block.position.z = (i - 2) * 1.2; // Stack the blocks
    counterweightGroup.add(block);
  }
...
function createCraneJib(length, material) {
  ...
  topBeam.position.set(length/2, 2, 0);
  
  const bottomBeam = new THREE.Mesh(
    new THREE.BoxGeometry(length, 0.8, 0.8),
    material
  );
  bottomBeam.position.set(length/2, 0, 0);
  
  jib.add(topBeam, bottomBeam);
  
  // Add vertical supports
  const numSupports = 15;
  const spacing = length / numSupports;
  ...
  for (let i = 0; i <= numSupports; i++) {
    ...
    verticalSupport.position.set(i * spacing, 1, 0);
    jib.add(verticalSupport);
    
    // Add lattice structure (X-bracing) between vertical supports
    if (i < numSupports) {
      const diagonalLength = Math.sqrt(spacing * spacing + 4);
      
      const diagonalGeom1 = new THREE.BoxGeometry(diagonalLength, 0.4, 0.4);
      diagonalGeom1.translate(diagonalLength/2, 0, 0);
      diagonalGeom1.rotateZ(Math.atan2(2, spacing));
      
      const diagonal1 = new THREE.Mesh(diagonalGeom1, material);
      diagonal1.position.set(i * spacing, 0, 0);
      
      const diagonalGeom2 = new THREE.BoxGeometry(diagonalLength, 0.4, 0.4);
      diagonalGeom2.translate(diagonalLength/2, 0, 0);
      diagonalGeom2.rotateZ(-Math.atan2(2, spacing));
      
      const diagonal2 = new THREE.Mesh(diagonalGeom2, material);
      diagonal2.position.set(i * spacing, 2, 0);
      
      jib.add(diagonal1, diagonal2);
    }
  }
...
function createTrolley(material) {
  ...
  const wheel1 = new THREE.Mesh(wheelGeometry, material);
  wheel1.position.set(-0.7, 0.8, 0);
  const wheel2 = new THREE.Mesh(wheelGeometry, material);
  wheel2.position.set(0.7, 0.8, 0);
  
  trolley.add(wheel1, wheel2);
...
// Animation function for the crane - add this to your animation loop
function animateTowerCrane(crane, delta) {
  if (!crane) return;
  
  const userData = crane.userData;
  const turntable = crane.getObjectByName('craneTurntable');
  const jib = turntable.getObjectByName('craneJib');
  const trolley = jib.getObjectByName('craneTrolley');
  const hookAndRope = trolley.getObjectByName('craneHook');
  
  // Smooth rotation of the turntable to target angle (180 degrees = Math.PI)
  const angleDiff = userData.targetRotation - turntable.rotation.y;
  if (Math.abs(angleDiff) > 0.01) {
    // Gradually adjust rotation speed for smooth acceleration/deceleration
    userData.rotationSpeed += angleDiff * 0.01;
    // Apply damping to prevent oscillation
    userData.rotationSpeed *= 0.95;
    // Apply speed limiter
    userData.rotationSpeed = THREE.MathUtils.clamp(userData.rotationSpeed, -0.02, 0.02);
    turntable.rotation.y += userData.rotationSpeed;
  } else {
    userData.rotationSpeed *= 0.9; // Slow down when close to target
  }
  ...
  if (Math.abs(hookDiff) > 0.1) {
    ...
    // Recreate the rope with new length
    if (hookAndRope) {
      // Update only the rope's scale to simulate extension/retraction
      const rope = hookAndRope.children[0];
      if (rope) {
        rope.scale.y = userData.hookHeight / 20; // 20 is the initial length
        rope.position.y = -userData.hookHeight / 2;
      }
  ...
  // Apply physics to the hook for realistic swinging
  // Gravity constant
  const gravity = 9.8;
  // Spring constant (rope stiffness)
  const k = 2.0;
  // Damping factor (air resistance)
  const damping = 0.05;
  
  // Calculate forces based on turntable rotation (centrifugal force)
  const centrifugalForce = userData.rotationSpeed * userData.hookHeight * 100;
...
// Function to control the crane - call these to operate the crane
function rotateCrane(crane, targetAngle) {
  if (!crane) return;
  crane.userData.targetRotation = targetAngle;
}

function moveTrolley(crane, position) {
  if (!crane) return;
  // Clamp position between 5 and 55 (to keep trolley on the jib)
  crane.userData.targetTrolleyPosition = THREE.MathUtils.clamp(position, 5, 55);
}

function setHookHeight(crane, height) {
  if (!crane) return;
  // Clamp height between 5 and 50
  crane.userData.targetHookHeight = THREE.MathUtils.clamp(height, 5, 50);
}
