...
function setupCraneControls(scene) {
  ...
  // Initialize crane user data if not already set
  crane.userData = crane.userData || {};
  crane.userData.isRotating = true;
  crane.userData.targetRotation = crane.rotation.y || 0;
  crane.userData.trolleyPosition = 30; // Default position
  crane.userData.hookHeight = 20; // Default height

  // Create UI container for crane controls
  ...
  controlsContainer.style.backgroundColor = 'rgba(50, 50, 50, 0.5)';
  ...
  centerButton.addEventListener('click', () => {
    crane.userData.isRotating = !crane.userData.isRotating;
    centerButton.innerHTML = crane.userData.isRotating ? '⏸' : '▶';
  });
  controlsContainer.appendChild(centerButton);

  // Create directional buttons in a circle
  // Left button (rotate counterclockwise)
  ...
  // Up button (move trolley outward)
  ...
  // Down button (move trolley inward)
  ...
  // Upper-right button (raise hook)
  const raiseButton = createButton('⬆', {
    ...buttonStyle,
    top: '25px',
    left: '95px'
  });
  ...
  controlsContainer.appendChild(raiseButton);

  // Lower-right button (lower hook)
  const lowerButton = createButton('⬇', {
    ...buttonStyle,
    top: '95px',
    left: '95px'
  });
  ...
  controlsContainer.appendChild(lowerButton);
  ...
  demoButton.style.fontSize = '24px';
  demoButton.title = "Run Demonstration";
  demoButton.addEventListener('click', () => {
    demonstrateCraneLift(scene, crane);
  });
  document.body.appendChild(demoButton);

  // Also keep keyboard controls for power users
  document.addEventListener('keydown', (event) => {
    switch (event.key) {
      ...
      case 'PageDown':
        setHookHeight(crane, crane.userData.hookHeight + 3);
        break;
      case ' ': // Space key toggles rotation
        crane.userData.isRotating = !crane.userData.isRotating;
        centerButton.innerHTML = crane.userData.isRotating ? '⏸' : '▶';
        break;
      case 'Enter': // Enter key starts demonstration
        demonstrateCraneLift(scene, crane);
        break;
    }
  });
...
// Helper function to create a button with given text and style
