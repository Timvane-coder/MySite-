...
generateStatistics() {
    ...
    switch (this.currentSheet.equation) {
        ...
        case 'custom_shopping':
            ...
            if (shoppingMetrics.discount > 0) {
                stats.push(`Discount: ${params.currency}${shoppingMetrics.discount.toFixed(2)} (${(params.discountRate * 100).toFixed(1)}%)`);
            }
            if (shoppingMetrics.tax > 0) {
                stats.push(`Tax: ${params.currency}${shoppingMetrics.tax.toFixed(2)} (${(params.taxRate * 100).toFixed(1)}%)`);
            }
            stats.push(`Total: ${params.currency}${shoppingMetrics.total.toFixed(2)}`);
            ...
            stats.push(`Estimated Shopping Time: ${shoppingMetrics.timeEstimate} minutes`);
