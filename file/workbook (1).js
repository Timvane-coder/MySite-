...
class StatisticalDistributions {
 ...
 static chiSquareInverse(p, df) {
    ...
    let x;
    if (df === 1) {
        x = Math.pow(StatisticalDistributions.normalInverse(p, 0, 1), 2);
    } else if (df === 2) {
        x = -2 * Math.log(1 - p);
    } else {
        // Wilson-Hilferty approximation for initial guess
        const h = 2 / (9 * df);
        const z = StatisticalDistributions.normalInverse(p, 0, 1);
        x = df * Math.pow(1 - h + z * Math.sqrt(h), 3);
        x = Math.max(0.1, x); // Ensure positive
    }

    // Newton-Raphson with better convergence criteria
    let iterations = 0;
    ...
    for (let i = 0; i < maxIterations; i++) {
        ...
        if (Math.abs(dfx) < 1e-20) break; // Avoid division by zero
        
        const newX = x - fx / dfx;
        
        // Ensure we stay positive and don't make huge jumps
        if (newX <= 0) {
            x = x / 2;
        } else if (newX > x * 10) {
            x = x * 2;
    ...
    if (iterations >= maxIterations) {
        console.warn(`chiSquareInverse convergence warning for p=${p}, df=${df}`);
        // Return reasonable approximation instead of error
        return df + Math.sqrt(2 * df) * StatisticalDistributions.normalInverse(p, 0, 1);
    }
...
static logNormalCDF(x, mu, sigma) {
    ...
    return this.normalCDF(z); // This should call normalCDF(z, 0, 1) or normalCDF with default params
}

static logNormalInverse(p, mu, sigma) {
    if (p <= 0) return 0;
    if (p >= 1) return Infinity;
    const z = this.normalInverse(p, 0, 1); // Explicitly use standard normal
    return Math.exp(mu + sigma * z);
}

// Pareto distribution (FIXED)
...
'lognormal': {
    ...
    estimateParams: (data) => {
        ...
        const sigma = Math.sqrt(Math.max(0.01, variance)); // Prevent zero/negative sigma
...
'pareto': {
    ...
    estimateParams: (data) => {
        ...
        // Prevent log(0) by ensuring x > xm
        const validData = positiveData.filter(x => x > xm);
        if (validData.length === 0) {
            // If all data equals xm, use a small adjustment
            const adjustedXm = xm * 0.999;
            const sumLogRatio = positiveData.reduce((sum, x) => sum + Math.log(Math.max(x, xm * 1.001) / adjustedXm), 0);
            const alpha = n / Math.max(0.001, sumLogRatio);
