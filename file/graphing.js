...
showSpecialFunctionPoints(equation, { function: func }) {
     ...
     */
/**
 * Create graph with coordinate points marked and domain/range info (UPDATED)
 */
async createGraphWithPoints(equation, calculator) {
    ...
    // Get function info
    const functionInfo = this.getFunctionInfo(equation);

    // Add coordinate points and enhanced line drawing
    this.markCoordinatePoints(ctx, equation, calculator);

    // Draw domain and range panel
    this.drawDomainRangeOnGraph(ctx, functionInfo.domain, functionInfo.range, calculator);
...
/**
 * Calculate domain for a function
 */
calculateDomain(equation, functionInfo) {
    // Check function type and return appropriate domain
    
    if (functionInfo.isLinear || functionInfo.isCubic || functionInfo.isExponential) {
        return {
            type: 'all_reals',
    ...
    if (functionInfo.isLogarithmic) {
        const { xCoeff, xShift } = functionInfo;
        const restriction = -xShift / xCoeff;
        
        return {
            type: 'restricted',
            description: xCoeff > 0 ? `x > ${restriction.toFixed(3)}` : `x < ${restriction.toFixed(3)}`,
            notation: xCoeff > 0 ? `(${restriction.toFixed(3)}, ∞)` : `(-∞, ${restriction.toFixed(3)})`,
            interval: xCoeff > 0 ? { min: restriction, max: Infinity } : { min: -Infinity, max: restriction }
        };
    }

    if (functionInfo.isSquareRoot) {
        const { xCoeff, xShift } = functionInfo;
        const restriction = -xShift / xCoeff;
    ...
    if (functionInfo.isAbsoluteValue) {
        return {
            type: 'all_reals',
            description: 'All real numbers',
            notation: '(-∞, ∞)',
    ...
    if (functionInfo.isTrigonometric) {
        const { function: func } = functionInfo;
        
        if (func === 'tan') {
            const { frequency, phase } = functionInfo;
            const asymptoteSpacing = Math.PI / Math.abs(frequency);
            const firstAsymptote = (-phase / frequency) + (Math.PI / (2 * frequency));
            
            return {
                type: 'restricted_periodic',
                description: `All real numbers except x = ${firstAsymptote.toFixed(3)} + n·${asymptoteSpacing.toFixed(3)}`,
                notation: 'ℝ \\ {asymptotes}',
                interval: { min: -Infinity, max: Infinity },
                exclusions: 'vertical asymptotes'
            };
    ...
    if (functionInfo.isRational) {
        return {
            type: 'restricted',
            description: 'All real numbers except where denominator = 0',
            notation: 'ℝ \\ {zeros of denominator}',
            interval: { min: -Infinity, max: Infinity },
            exclusions: 'Check denominator zeros'
        };
    }

    if (functionInfo.isSpecial) {
        return {
            type: 'all_reals',
            description: 'All real numbers',
            notation: '(-∞, ∞)',
...
/**
 * Calculate range for a function
 */
calculateRange(equation, functionInfo) {
    if (functionInfo.isLinear) {
        const { slope } = functionInfo;
        
        if (slope === 0) {
            const { intercept } = functionInfo;
            return {
                type: 'single_value',
                description: `y = ${intercept}`,
                notation: `{${intercept}}`,
                interval: { min: intercept, max: intercept }
            };
        } else {
            return {
                type: 'all_reals',
    ...
    if (functionInfo.isQuadratic) {
        const { a, k } = functionInfo;
        
        if (a > 0) {
            return {
                type: 'bounded_below',
                description: `y ≥ ${k.toFixed(3)}`,
                notation: `[${k.toFixed(3)}, ∞)`,
                interval: { min: k, max: Infinity },
                extremum: { type: 'minimum', value: k }
            };
        } else {
            return {
                type: 'bounded_above',
                description: `y ≤ ${k.toFixed(3)}`,
                notation: `(-∞, ${k.toFixed(3)}]`,
                interval: { min: -Infinity, max: k },
                extremum: { type: 'maximum', value: k }
            };
        }
    }

    if (functionInfo.isCubic) {
        return {
            type: 'all_reals',
            description: 'All real numbers',
            notation: '(-∞, ∞)',
    ...
    if (functionInfo.isExponential) {
        const { coefficient, exponentCoeff } = functionInfo;
        
        if (coefficient * exponentCoeff > 0) {
            // Growing exponential
            return {
                type: 'bounded_below',
                description: 'y > 0',
            ...
            return {
                type: 'bounded_above',
                description: 'y < 0 or y > 0',
                notation: coefficient > 0 ? '(0, ∞)' : '(-∞, 0)',
                interval: coefficient > 0 ? { min: 0, max: Infinity } : { min: -Infinity, max: 0 },
                asymptote: 'y = 0 (horizontal asymptote)'
            };
        }
    }
    ...
    if (functionInfo.isTrigonometric) {
        const { function: func, amplitude, verticalShift } = functionInfo;
        ...
        if (func === 'sin' || func === 'cos') {
            ...
            return {
                type: 'bounded',
                description: `${minY.toFixed(3)} ≤ y ≤ ${maxY.toFixed(3)}`,
                notation: `[${minY.toFixed(3)}, ${maxY.toFixed(3)}]`,
                interval: { min: minY, max: maxY },
                periodic: true
            };
        } else if (func === 'tan') {
            return {
            ...
            return {
                type: 'bounded',
                description: '-π/2 ≤ y ≤ π/2',
                notation: '[-π/2, π/2]',
                interval: { min: -Math.PI / 2, max: Math.PI / 2 }
            };
        } else if (func === 'acos') {
            return {
                type: 'bounded',
    ...
    if (functionInfo.isAbsoluteValue) {
        const { coefficient, verticalShift } = functionInfo;
        
        if (coefficient > 0) {
            return {
                type: 'bounded_below',
                description: `y ≥ ${verticalShift}`,
                notation: `[${verticalShift}, ∞)`,
                interval: { min: verticalShift, max: Infinity }
            };
        } else {
            return {
                type: 'bounded_above',
                description: `y ≤ ${verticalShift}`,
                notation: `(-∞, ${verticalShift}]`,
                interval: { min: -Infinity, max: verticalShift }
            };
        }
    }
    ...
    if (functionInfo.isRational) {
        return {
            type: 'restricted',
            description: 'Depends on function behavior',
            notation: 'Check horizontal asymptotes',
            interval: { min: -Infinity, max: Infinity },
            note: 'May exclude horizontal asymptote values'
        };
    }
...
/**
 * Get complete function information including domain and range
 */
getFunctionInfo(equation) {
    // Try each parser
    let functionInfo = this.parseQuadratic(equation);
    if (functionInfo.isQuadratic) {
        functionInfo.type = 'quadratic';
    } else {
        functionInfo = this.parseLinear(equation);
        if (functionInfo.isLinear) {
            functionInfo.type = 'linear';
        } else {
            functionInfo = this.parseCubic(equation);
            if (functionInfo.isCubic) {
                functionInfo.type = 'cubic';
            } else {
                functionInfo = this.parseExponential(equation);
                if (functionInfo.isExponential) {
                    functionInfo.type = 'exponential';
                } else {
                    functionInfo = this.parseLogarithmic(equation);
                    if (functionInfo.isLogarithmic) {
                        functionInfo.type = 'logarithmic';
                    } else {
                        functionInfo = this.parseTrigonometric(equation);
                        if (functionInfo.isTrigonometric) {
                            functionInfo.type = 'trigonometric';
                        } else {
                            functionInfo = this.parseAbsoluteValue(equation);
                            if (functionInfo.isAbsoluteValue) {
                                functionInfo.type = 'absolute_value';
                            } else {
                                functionInfo = this.parseSquareRoot(equation);
                                if (functionInfo.isSquareRoot) {
                                    functionInfo.type = 'square_root';
                                } else {
                                    functionInfo = this.parseRational(equation);
                                    if (functionInfo.isRational) {
                                        functionInfo.type = 'rational';
                                    } else {
                                        functionInfo = this.parseSpecialFunction(equation);
                                        if (functionInfo.isSpecial) {
                                            functionInfo.type = 'special';
                                        }
                                    }
                                }
                            }
    ...
    functionInfo.domain = this.calculateDomain(equation, functionInfo);
    functionInfo.range = this.calculateRange(equation, functionInfo);

    return functionInfo;
}

/**
 * Display domain and range information
 */
displayDomainRange(domain, range) {
    console.log(`\n📏 DOMAIN & RANGE:`);
    console.log(`   Domain:  ${domain.description}`);
    console.log(`            ${domain.notation}`);
    if (domain.exclusions) {
        console.log(`            Exclusions: ${domain.exclusions}`);
    }
    
    console.log(`   Range:   ${range.description}`);
    console.log(`            ${range.notation}`);
    if (range.extremum) {
        console.log(`            ${range.extremum.type}: ${range.extremum.value.toFixed(3)}`);
    }
    if (range.asymptote) {
        console.log(`            ${range.asymptote}`);
    }
    if (range.periodic) {
        console.log(`            (Periodic function)`);
    }
    if (range.note) {
        console.log(`            Note: ${range.note}`);
    }
}

/**
 * Draw domain and range information on graph
 */
drawDomainRangeOnGraph(ctx, domain, range, calculator) {
    const panelX = calculator.width - 220;
    const panelY = 10;
    ...
    ctx.font = 'bold 13px Arial';
    ctx.textAlign = 'left';
    ctx.fillText('Domain & Range', panelX + 10, panelY + 20);

    // Domain
    ctx.font = 'bold 11px Arial';
    ctx.fillText('Domain:', panelX + 10, panelY + 40);
    
    ctx.font = '10px Arial';
    const domainLines = this.wrapText(ctx, domain.description, panelWidth - 20);
    domainLines.forEach((line, index) => {
        ctx.fillText(line, panelX + 10, panelY + 53 + index * 12);
    });

    // Range
    ctx.font = 'bold 11px Arial';
    ctx.fillText('Range:', panelX + 10, panelY + 80);
    
    ctx.font = '10px Arial';
    const rangeLines = this.wrapText(ctx, range.description, panelWidth - 20);
    rangeLines.forEach((line, index) => {
        ctx.fillText(line, panelX + 10, panelY + 93 + index * 12);
    });
}
...
/**
 * Show key points for linear equations (UPDATED WITH DOMAIN/RANGE)
 */
showLinearPoints(equation, linearInfo) {
    ...
    // Get function info with domain and range
    const fullInfo = this.getFunctionInfo(equation);
    this.displayDomainRange(fullInfo.domain, fullInfo.range);
...
/**
 * Show key points for quadratic equations (UPDATED WITH DOMAIN/RANGE)
 */
showQuadraticPoints(equation, quadraticInfo) {
    const { a, h, k, form } = quadraticInfo;
...
/**
 * Show key points for cubic equations (UPDATED WITH DOMAIN/RANGE)
 */
showCubicPoints(equation, cubicInfo) {
    const { a, b, c, d } = cubicInfo;
    ...
    if (discriminant > 0) {
        ...
        console.log(`\n🔍 Critical Points (local extrema):`);
        console.log(`   Point 1: (${x1.toFixed(3)}, ${y1.toFixed(3)}) - ${y1 > y2 ? 'Local maximum' : 'Local minimum'}`);
        console.log(`   Point 2: (${x2.toFixed(3)}, ${y2.toFixed(3)}) - ${y2 > y1 ? 'Local maximum' : 'Local minimum'}`);
    } else if (discriminant === 0) {
        const x = -2 * b / (6 * a);
        const y = a * x ** 3 + b * x ** 2 + c * x + d;
        console.log(`\n🔍 Inflection Point: (${x.toFixed(3)}, ${y.toFixed(3)})`);
    } else {
        console.log(`\n🔍 No critical points (monotonic function)`);
    }
    ...
    // End behavior
    console.log(`\n📈 End Behavior:`);
    console.log(`   As x → -∞: y → ${a > 0 ? '-∞' : '+∞'}`);
    console.log(`   As x → +∞: y → ${a > 0 ? '+∞' : '-∞'}`);
}

/**
 * Show key points for exponential equations (UPDATED WITH DOMAIN/RANGE)
 */
showExponentialPoints(equation, exponentialInfo) {
    const { coefficient, base, exponentCoeff, exponentShift } = exponentialInfo;
    
    console.log(`📊 Exponential Function Analysis:`);
    console.log(`   Equation: ${equation}`);
    console.log(`   Coefficient: ${coefficient}`);
    console.log(`   Base: ${base === Math.E ? 'e (natural)' : base.toFixed(3)}`);
    console.log(`   Type: ${coefficient * exponentCoeff > 0 ? 'Exponential growth 📈' : 'Exponential decay 📉'}`);
    ...
    // Growth/decay rate
    if (base === Math.E) {
        const growthRate = exponentCoeff * 100;
        console.log(`   Continuous ${growthRate > 0 ? 'growth' : 'decay'} rate: ${Math.abs(growthRate).toFixed(2)}%`);
    }
    ...
    // Doubling/halving time for growth/decay
    if (coefficient * exponentCoeff > 0) {
        const doublingTime = Math.log(2) / (Math.log(base) * exponentCoeff);
        console.log(`\n⏱️  Doubling time: ${doublingTime.toFixed(3)} units`);
    } else {
        const halvingTime = Math.log(0.5) / (Math.log(base) * exponentCoeff);
        console.log(`\n⏱️  Half-life: ${Math.abs(halvingTime).toFixed(3)} units`);
    }
}

/**
 * Show key points for logarithmic equations (UPDATED WITH DOMAIN/RANGE)
 */
showLogarithmicPoints(equation, logarithmicInfo) {
    const { coefficient, base, xCoeff, xShift } = logarithmicInfo;
    
    console.log(`📊 Logarithmic Function Analysis:`);
    console.log(`   Equation: ${equation}`);
    console.log(`   Coefficient: ${coefficient}`);
    console.log(`   Base: ${base === Math.E ? 'e (natural log)' : base.toFixed(3)}`);
    ...
    // X-intercept (when y = 0)
    // coefficient * log_base(xCoeff * x + xShift) = 0
    // log_base(xCoeff * x + xShift) = 0
    // xCoeff * x + xShift = 1
    const xIntercept = (1 - xShift) / xCoeff;
    if (xIntercept > domainStart && xIntercept >= this.calculator.xMin && xIntercept <= this.calculator.xMax) {
        console.log(`🎯 X-intercept: (${xIntercept.toFixed(4)}, 0)`);
    }

    // Sample points
    console.log(`\n📍 Key Points:`);
    const samplePoints = [0.1, 0.5, 1, 2, 3, 5, 10];
    const validPoints = samplePoints.map(offset => domainStart + offset).filter(x => x > domainStart);
    ...
    // Behavior
    console.log(`\n📈 Behavior:`);
    console.log(`   As x → ${domainStart.toFixed(3)}⁺: y → ${coefficient > 0 ? '-∞' : '+∞'}`);
    console.log(`   As x → +∞: y → ${coefficient > 0 ? '+∞' : '-∞'}`);
}

/**
 * Show key points for trigonometric equations (UPDATED WITH DOMAIN/RANGE)
 */
showTrigonometricPoints(equation, trigInfo) {
    const { function: func, amplitude, frequency, phase, verticalShift } = trigInfo;
    
    console.log(`📊 Trigonometric Function Analysis:`);
    console.log(`   Function: ${func}`);
    console.log(`   Amplitude: ${amplitude}`);
    console.log(`   Frequency: ${frequency}`);
    console.log(`   Phase shift: ${phase} (${(phase * 180 / Math.PI).toFixed(1)}°)`);
    console.log(`   Vertical shift: ${verticalShift}`);
    ...
    console.log(`\n⏱️  Period: ${period.toFixed(4)} (${(period * 180 / Math.PI).toFixed(1)}°)`);
    console.log(`   Frequency: ${Math.abs(frequency).toFixed(3)} cycles per 2π`);

    // Midline and amplitude info
    if (func === 'sin' || func === 'cos') {
        console.log(`\n📏 Midline: y = ${verticalShift}`);
        console.log(`   Maximum: y = ${(verticalShift + Math.abs(amplitude)).toFixed(3)}`);
        console.log(`   Minimum: y = ${(verticalShift - Math.abs(amplitude)).toFixed(3)}`);
    } else if (func === 'tan') {
        const asymptoteSpacing = period;
        const firstAsymptote = -phase / frequency + Math.PI / (2 * frequency);
        console.log(`\n⚠️  Vertical asymptotes at:`);
        console.log(`   x = ${firstAsymptote.toFixed(3)} + n·${asymptoteSpacing.toFixed(3)}, where n is any integer`);
    }

    // Sample points (one complete period)
    console.log(`\n📍 Key Points (one period):`);
    const numPoints = func === 'tan' ? 3 : 5;
    const startX = -phase / frequency;
    
    for (let i = 0; i <= numPoints; i++) {
        const x = startX + i * period / numPoints;
        if (x >= this.calculator.xMin && x <= this.calculator.xMax) {
            let y;
            const arg = frequency * x + phase;
            ...
            if (y !== undefined && !isNaN(y) && isFinite(y) && 
                y >= this.calculator.yMin && y <= this.calculator.yMax) {
                const pointType = this.getTrigPointType(func, i, numPoints, amplitude, verticalShift, y);
                console.log(`   (${x.toFixed(4)}, ${y.toFixed(4)}) ${pointType}`);
            }
        }
    }

    // Special angles for standard trig functions
    if (func === 'sin' || func === 'cos') {
        console.log(`\n🎯 Special Features:`);
        console.log(`   Zero crossings occur at midline (y = ${verticalShift})`);
        console.log(`   Maxima and minima alternate every half period`);
    }
}

/**
 * Helper to identify trig point types
 */
getTrigPointType(func, index, totalPoints, amplitude, verticalShift, yValue) {
    if (func === 'sin' || func === 'cos') {
        const diff = yValue - verticalShift;
        if (Math.abs(diff - Math.abs(amplitude)) < 0.01) return '← Maximum';
        if (Math.abs(diff + Math.abs(amplitude)) < 0.01) return '← Minimum';
...
/**
 * Show key points for absolute value equations (UPDATED WITH DOMAIN/RANGE)
 */
showAbsoluteValuePoints(equation, absInfo) {
    if (absInfo.isMultiple) {
        console.log(`📊 Multiple Absolute Value Function:`);
        console.log(`   Equation: ${equation}`);
        console.log(`   📍 Complex piecewise function - check graph for visualization`);
    ...
    const { coefficient, xCoeff, xShift, verticalShift } = absInfo;
    ...
    const vertexX = -xShift / xCoeff;
    const vertexY = verticalShift;
    console.log(`\n🎯 Vertex (Corner point): (${vertexX.toFixed(3)}, ${vertexY})`);
    
    // Slopes
    const rightSlope = coefficient * xCoeff;
    const leftSlope = -coefficient * xCoeff;
    console.log(`\n📐 Slopes:`);
    console.log(`   Left branch: ${leftSlope}`);
    console.log(`   Right branch: ${rightSlope}`);
    
    // Opening direction
    console.log(`   Opens: ${coefficient > 0 ? 'Upward ∨' : 'Downward ∧'}`);
    ...
    if (coefficient > 0 && verticalShift < 0) {
        const xInt1 = (-verticalShift / coefficient - xShift) / xCoeff;
        const xInt2 = (verticalShift / coefficient - xShift) / xCoeff;
        console.log(`\n🎯 X-intercepts:`);
        console.log(`   (${xInt1.toFixed(3)}, 0)`);
        console.log(`   (${xInt2.toFixed(3)}, 0)`);
    } else if (coefficient < 0 && verticalShift > 0) {
        const xInt1 = (-verticalShift / coefficient - xShift) / xCoeff;
        const xInt2 = (verticalShift / coefficient - xShift) / xCoeff;
        console.log(`\n🎯 X-intercepts:`);
        console.log(`   (${xInt1.toFixed(3)}, 0)`);
...
/**
 * Show key points for square root equations (UPDATED WITH DOMAIN/RANGE)
 */
showSquareRootPoints(equation, sqrtInfo) {
    const { coefficient, xCoeff, xShift, verticalShift } = sqrtInfo;
    ...
    const startY = verticalShift;
    console.log(`\n🎯 Starting point: (${startX.toFixed(3)}, ${startY})`);
    console.log(`   (Where the radicand equals zero)`);

    // Direction
    console.log(`\n📈 Direction:`);
    if (xCoeff > 0) {
        console.log(`   Extends to the right →`);
    } else {
        console.log(`   Extends to the left ←`);
    }
    
    if (coefficient > 0) {
        console.log(`   Curves upward ↗`);
    } else {
        console.log(`   Curves downward ↘`);
    }
    ...
    // Rate of change
    console.log(`\n📊 Behavior:`);
    console.log(`   Rate of change decreases as x increases`);
    console.log(`   Function is concave ${coefficient > 0 ? 'down' : 'up'}`);
}

/**
 * Show key points for rational equations (UPDATED WITH DOMAIN/RANGE)
 */
showRationalPoints(equation, rationalInfo) {
    const { numerator, denominator } = rationalInfo;
    ...
    // Try to find vertical asymptotes (where denominator = 0)
    console.log(`\n⚠️  Vertical Asymptotes:`);
    console.log(`   Occur where denominator = 0`);
    ...
    if (denominator === 'x') {
        ...
        console.log(`   Solve: ${denominator} = 0`);
    }

    // Horizontal asymptote analysis
    console.log(`\n📏 Horizontal Asymptote:`);
    console.log(`   Compare degrees of numerator and denominator:`);
    console.log(`   • If deg(num) < deg(den): y = 0`);
    console.log(`   • If deg(num) = deg(den): y = ratio of leading coefficients`);
    console.log(`   • If deg(num) > deg(den): No horizontal asymptote (oblique possible)`);

    // Y-intercept
    console.log(`\n🎯 Y-intercept:`);
    console.log(`   Substitute x = 0 into the function`);
    console.log(`   (If denominator ≠ 0 at x = 0)`);

    // X-intercepts
    console.log(`\n🎯 X-intercepts:`);
    console.log(`   Occur where numerator = 0 (and denominator ≠ 0)`);

    // Sample points
    console.log(`\n📍 Note: Evaluate function at specific x-values`);
    console.log(`   Avoid values where denominator = 0`);
    console.log(`   📊 Check individual graph for detailed visualization`);

    // Special behavior
    console.log(`\n📈 Special Behavior:`);
    console.log(`   Check for holes (removable discontinuities)`);
    console.log(`   Analyze end behavior using asymptotes`);
}

/**
 * Show key points for special functions (UPDATED WITH DOMAIN/RANGE)
 */
showSpecialFunctionPoints(equation, specialInfo) {
    const { function: func } = specialInfo;
    ...
    switch (func) {
        case 'floor':
            console.log(`\n📖 Description: Greatest Integer Function`);
            console.log(`   Returns: Largest integer ≤ x`);
            console.log(`   Symbol: ⌊x⌋`);
            console.log(`\n⚠️  Discontinuities: At every integer`);
            console.log(`   Left-continuous at integers`);
            console.log(`\n📍 Examples:`);
            console.log(`   ⌊2.7⌋ = 2`);
            console.log(`   ⌊-1.3⌋ = -2`);
            console.log(`   ⌊5⌋ = 5`);
        ...
        case 'ceil':
            ...
            console.log(`   Returns: Smallest integer ≥ x`);
            console.log(`   Symbol: ⌈x⌉`);
            console.log(`\n⚠️  Discontinuities: At every integer`);
            console.log(`   Right-continuous at integers`);
            console.log(`\n📍 Examples:`);
            console.log(`   ⌈2.7⌉ = 3`);
            console.log(`   ⌈-1.3⌉ = -1`);
            console.log(`   ⌈5⌉ = 5`);
        ...
        case 'sign':
            console.log(`\n📖 Description: Sign (Signum) Function`);
            console.log(`   Returns: -1, 0, or 1`);
            console.log(`\n📏 Definition:`);
            console.log(`   sgn(x) = -1  if x < 0`);
            console.log(`   sgn(x) =  0  if x = 0`);
            console.log(`   sgn(x) =  1  if x > 0`);
            console.log(`\n⚠️  Discontinuity: At x = 0`);
            console.log(`\n📍 Key Points:`);
            console.log(`   (-1, -1), (0, 0), (1, 1)`);
            break;
            
        case 'max':
            console.log(`\n📖 Description: Maximum Function (ReLU)`);
            console.log(`   Returns: max(0, x)`);
            console.log(`   Used in: Machine learning, neural networks`);
            console.log(`\n📏 Definition:`);
            console.log(`   ReLU(x) = 0  if x < 0`);
            console.log(`   ReLU(x) = x  if x ≥ 0`);
            console.log(`\n🎯 Corner Point: (0, 0)`);
            console.log(`\n📐 Slopes:`);
            console.log(`   Left: 0`);
            console.log(`   Right: 1`);
    ...
    console.log(`   Step-like or piecewise linear`);
    console.log(`   May have jump discontinuities`);
    console.log(`   📈 Check individual graph for detailed visualization`);
}
