...
const getYoutubeMP4 = async (text, onProgress) => {  
    return new Promise(async (resolve, reject) => {  
        try {  
            let response = {};  
...
// Stream YouTube video in MP4 format with progress tracking
const streamYoutubeMP4 = async (text, onProgress, outputStream) => {
    try {
        // Get video information
        const { result: videoInfo } = await getYoutubeVideoInfo(text);

        if (!videoInfo) throw new Error("❌ Failed to retrieve video information.");

        // Stream video directly from YouTube
        const videoStream = ytdl(videoInfo.videoId, {
        ...
        // Pipe video directly to output (no full download in memory)
        ...
        console.error("❌ Error in streamYoutubeMP4:", err.message);
    }
};


// Function to get YouTube video in MP4 format with progress tracking
...
module.exports = { getYoutubeVideoInfo, getYoutubeMP4, getYoutubeMP3,streamYoutubeMP4 }
